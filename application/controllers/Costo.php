...
class Costo extends CI_Controller{
    ...
    function add(){
        if($this->acceso(136)){
            $this->form_validation->set_rules('costo_descripcion','costo_descripcion','trim|required', array('required' => 'Este Campo no debe ser vacio'));
            $estado = 1;
            if($this->form_validation->run()){
                $params = array(
                    'costo_descripcion' => $this->input->post('costo_descripcion'),
                ...
                $this->Costo_model->add_costo($params);
                redirect('costo/index');
            }else{
                $estado_tipo = 1;
                $data['categorias'] = $this->Categoria_costo_model->get_all_categorias("");
                $data['estados'] = $this->Estado_model->get_estado_tipo($estado_tipo);
                $data['page_title'] = "Costo";
                $data['_view'] = 'costo/add';
                $this->load->view('layouts/main',$data);
            }
        }
    }  
