            ...
            if (inv != null){
                ...
                for (var i = 0; i < tamanio ; i++){
                    ...
                    if (categoria != inv[i]["categoria_nombre"]){
                        ...
                        total_egreso = Number(Number(inv[i]["cantidad_mantenimiento"])+Number(inv[i]["cantidad_proyecto"])+Number(inv[i]["cantidad_parque"])+Number(inv[i]["cantidad_venta"])+Number(inv[i]["cantidad_traspaso"])+Number(inv[i]["cantidad_mortandad"])+Number(inv[i]["cantidad_perdida"])).toFixed(0);
                        total_egresomoney = Number(Number(inv[i]["total_ventamantenimiento"])+Number(inv[i]["total_ventaproyecto"])+Number(inv[i]["total_ventaparque"])+Number(inv[i]["total_ventavarios"])+Number(inv[i]["total_ventatraspaso"])+Number(inv[i]["total_ventamortandad"]));
                        ...
                        html += "             	<td "+margen+" class='text-right'><font size='1'><b>"+Number(Number(total_egresomoney)).toFixed(2)+"</b></font></td>";
    ...
    if(tam>0){
      /* **************INICIO Generar Excel JavaScript************** */
        ...
        for (var i = 0; i < tam; i++) {
            ...
            row += '"' +Number(Number(Number(producto_total)-Number(Number(total_egreso)+Number(inv[i]["cantidad_mortandad"])+Number(inv[i]["cantidad_perdida"])))*Number(inv[i]["producto_costo"])).toFixed(2)+ '",';
